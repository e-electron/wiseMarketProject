  ...
  <div class="prodetail">
    ...
    <div class="comment">
      ...
      <div class="comment-list">
        <div class="comment-item" v-for="item in conmmentList" :key="item.comment_id">
          <div class="top">
...
import loginConfrim from '@/mixins/loginConfrim'
export default {
  name: 'ProDetail',
  mixins: [loginConfrim],
  components: {
  ...
  created () {
    this.getDetail()
    this.getComments()
    this.cartTotal = this.detail.cartTotal
  },
  computed: {
    goodsId () {
  ...
  methods: {
    ...
    goBuyNow () {
      // 未登录的处理:需要弹出一个确认框 您当前的操作需要登录才能继续
      if (this.loginConfirm()) {
        return
      }
      this.$router.push({
